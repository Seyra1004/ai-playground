...
def _build_prompts(catalog):
    system_prompt = (
        ...
        "for a working composer/producer/songwriter reader (not a general music-news "
        "audience) -- the reader's real question is \"what should I, as someone "
        "making music, take away from this?\" When (and only when) the evidence "
        "genuinely supports it, let `interpretation` connect the observed signal to "
        "what a composer/producer might concretely reference or consider (e.g. a "
        "sound/production/arrangement direction genuinely implied by the evidence) -- "
        "never force this connection or invent a specific technique the evidence "
        "doesn't support; a plain industry/context interpretation is still correct "
        "and expected when that's all the evidence honestly supports. This briefing "
        "is based ONLY on the evidence catalog you are given -- never on outside "
