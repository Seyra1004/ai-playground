"""Producer Intelligence synthesis CLI -- Stage 3b of scripts/
run_daily_pipeline.sh, also runnable standalone for manual/local use:

    .venv\\Scripts\\python.exe scripts\\run_daily_producer_intelligence.py

Wires argument parsing -> DB connection -> report.producer_orchestrator
...
build_dashboard_data_v2 is what supplies the evidence) -- in the pipeline
this runs after Stage 2c (derived signals) and Stage 3 (report
generation); run scripts/run_daily_report.py first if invoking manually.
Does not send anything to Kakao. Not a required pipeline stage -- see
run_daily_pipeline.sh's own Stage 3b comment for why a failure here never
fails the overall daily run.
...
def _generate_run_id():
    """producer-intelligence-<UTC timestamp>-<short suffix> -- distinct
    prefix from every other CLI's run_ids so none collide in the shared
    runs.run_id UNIQUE space."""
    timestamp = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    suffix = secrets.token_hex(3)
    return f"producer-intelligence-{timestamp}-{suffix}"


def _parse_args(argv):
    parser = argparse.ArgumentParser(description="Run SUPER NEWS's daily Producer Intelligence synthesis.")
    parser.add_argument(
...
def main(argv=None):
    ...
    try:
        try:
            result = run_daily_producer_intelligence(conn, run_id, report_date_kst)
