...
# ---- EMERGENCY QUALITY RECOVERY PASS (2026-08-17): lenient JSON
# extraction + one repair retry -- confirmed real intermittent defect: the
# SAME candidate pool succeeded on one real call and produced a fenced/
# prose-wrapped or truncated result on the next real call. --------------


def test_result_text_wrapped_in_markdown_fence_is_recovered():
    stdout = json.dumps({
        "is_error": False,
        "result": '```json\n{"greeting": "fenced"}\n```',
        "usage": {},
    ...
    with patch("subprocess.run", return_value=_FakeCompletedProcess(0, stdout=stdout)):
        response = ClaudeCLIStructuredLLM(
            model="claude-sonnet-5", executable="claude", timeout_seconds=30,
        ).generate_structured("sys", "user", _SCHEMA)
    assert response.parsed == {"greeting": "fenced"}


def test_result_text_with_prose_before_and_after_json_is_recovered():
    stdout = json.dumps({
        "is_error": False,
        "result": 'Here is the JSON you asked for:\n{"greeting": "prose"}\nLet me know if you need anything else.',
        "usage": {},
    })
    with patch("subprocess.run", return_value=_FakeCompletedProcess(0, stdout=stdout)):
        response = ClaudeCLIStructuredLLM(
    ...
    assert response.parsed == {"greeting": "prose"}


def test_invalid_json_retries_once_and_succeeds_on_repair_attempt():
    bad_stdout = json.dumps({"is_error": False, "result": "not valid json at all", "usage": {}})
    good_stdout = _success_stdout({"greeting": "repaired"})
    calls = []

    def _fake_run(cmd, **kwargs):
        calls.append(cmd)
        if len(calls) == 1:
            return _FakeCompletedProcess(0, stdout=bad_stdout)
        return _FakeCompletedProcess(0, stdout=good_stdout)
    ...
    assert response.parsed == {"greeting": "repaired"}
    assert len(calls) == 2
    # The retry attempt's --system-prompt must carry the repair instruction.
    second_system_prompt = calls[1][calls[1].index("--system-prompt") + 1]
    assert "valid JSON" in second_system_prompt
    assert "sys" in second_system_prompt  # original system prompt preserved, not replaced


def test_invalid_json_on_both_attempts_raises_after_exactly_one_retry():
    bad_stdout = json.dumps({"is_error": False, "result": "still not valid json", "usage": {}})
    calls = []
...
def test_clean_json_never_triggers_a_retry():
    calls = []

    def _fake_run(cmd, **kwargs):
        calls.append(cmd)
        return _FakeCompletedProcess(0, stdout=_success_stdout({"greeting": "clean"}))
    ...
    assert response.parsed == {"greeting": "clean"}
    assert len(calls) == 1
